...
type ChatMessage = { id: number; internId: number; senderName: string; role: string; content: string; isRead: boolean; createdAt: string | Date };
...
function MessagesPage({ internId }: { internId: number }) {
  const { internName } = useContext(InternContext);
  ...
  const load = async () => {
    try {
      const res = await fetch(`/api/intern/messages?internId=${internId}`);
      if (!res.ok) return;
  ...
  const sendMessage = async () => {
    ...
    const optimistic: ChatMessage = { id: optimisticId, internId, senderName: internName, role: "intern", content, isRead: false, createdAt: new Date().toISOString() };
    ...
    try {
      const res = await fetch(`/api/intern/messages?internId=${internId}`, {
        ...
        body: JSON.stringify({ internId, content }),
      });
      ...
      <aside className="border-b border-[#e5edf4] md:border-b-0 md:border-r">
        ...
        <button className="flex w-full items-center gap-3 border-b border-[#e9eef4] bg-[#ecf9f7] px-4 py-4 text-left transition hover:bg-[#f7fbfb]">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 truncate text-xs text-[#8095b0]">{lastMessage ? lastMessage.content : "Supervisor"}</p>
          </div>
      ...
      <div className="flex min-w-0 flex-col">
        ...
        <div className="border-t border-[#e8eef4] p-4">
          <div className="flex items-center gap-2 rounded-xl border border-[#dce6ef] px-3 py-2">
            ...
            <input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") sendMessage(); }} placeholder={`Message ${supervisorName.split(" ")[0]}...`} className="min-w-0 flex-1 bg-transparent px-1 text-sm text-[#2b4664] outline-none placeholder:text-[#98a8be]" />
            <button onClick={sendMessage} disabled={!message.trim() || sending} className="grid h-8 w-8 place-items-center rounded-lg bg-[#0d9f99] text-white hover:bg-[#078e89] disabled:opacity-50">{sending ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <Send className="h-4 w-4" />}</button>
          </div>
        </div>
      </div>
    </div>
...
export function InternClient({
  ...
  const content = () => {
    ...
    if (activePage === "Messages") return <MessagesPage internId={internId} />;
        ...
        <header className="sticky top-0 z-30 flex h-[78px] items-center border-b border-[#e3eaf2] bg-white/95 px-5 backdrop-blur lg:px-8">
          ...
          <div className="relative ml-2 border-l border-[#e8eef4] pl-3 sm:ml-4">
            ...
            {profileOpen && <div className="absolute right-0 top-12 z-40 w-[190px] overflow-hidden rounded-xl border border-[#e0e8f1] bg-white p-1.5 shadow-xl animate-pop">
              ...
              <button onClick={signOut} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-xs font-semibold text-[#c75d65] hover:bg-[#fff5f5]"><LogOut className="h-4 w-4" /> Sign out</button>
            </div>}
          </div>
        </header>
